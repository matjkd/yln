    ...
    function edit_content() {
        ...
        if ($this->form_validation->run() == FALSE) { // validation hasn'\t been passed
            ...
            $this->upload_image($id);
 			...
 			//get menu link
				 $menuLink = $this->content_model->get_menu($id);
				 foreach($menuLink as $row):
...
//delete files from server
        ...
        if ($this->form_validation->run() == FALSE) { // validation hasn'\t been passed
            ...
            $blog_id = mysql_insert_id();
                ...
                $this->upload_image();
				 ...
				 //get menu link
				 $menuLink = $this->content_model->get_menu($blog_id);
    ...
    function add_gallery_content() {
    	$data['selectedGallery'] = end($this->uri->segment_array());
        $data['slideshowtoggle'] = "off";
        $data['main_content'] = "admin/add_content";
        $data['seo_links'] = $this->content_model->get_seo_links();
        $data['captcha'] = $this->captcha_model->initiate_captcha();
	...
	function create_gallery_for_event($eventID) {
	$galleryName = $this->input->post('galleryname');
	
	echo $galleryName;
	
	//Create Gallery
	$this->gallery_model->create_new_gallery($galleryName);
	$galleryID = mysql_insert_id();
	
	//Assign  gallery to eventID
	$this->content_model->link_gallery_to_event($eventID, $galleryID );
	
	redirect( $this->agent->referrer(), 'refresh');
	
	}
	function create_gallery_for_page($pageID) {
	$galleryName = $this->input->post('galleryname');
	...
	//Assign  gallery to eventID
	$this->content_model->link_gallery_to_page($pageID, $galleryID );
	
	redirect( $this->agent->referrer(), 'refresh');
	
	}
	
	function assign_gallery_to_page($id) {
		
		$pageID = $id;
		$galleryID = $this->input->post('gallery');
		$this->content_model->link_gallery_to_page($pageID, $galleryID );
		redirect( $this->agent->referrer(), 'refresh');
		
	}
	
	function assign_gallery_to_event($id) {
